...
@Component()
public class MongobiQueryProvider extends QueryProvider {
    @Resource
    ...
    private SQLObj buildSortField(DeSortField f, SQLObj tableObj, int index) {
        ...
        if (f.getDeExtractType() == 1) {
            if (f.getDeType() == 2 || f.getDeType() == 3) {
                ...
                fieldName = String.format(MongoConstants.DATE_FORMAT, originField, MongoConstants.DEFAULT_DATE_FORMAT);
            }
        } else if (f.getDeExtractType() == 0) {
            if (f.getDeType() == 2) {
                fieldName = String.format(MongoConstants.CAST, originField, MongoConstants.DEFAULT_INT_FORMAT);
            ...
            if (f.getDeType() == 1) {
                ...
                fieldName = String.format(MongoConstants.FROM_UNIXTIME, cast, MongoConstants.DEFAULT_DATE_FORMAT);
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    if (x.getDeType() == 2 || x.getDeType() == 3) {
                        originField = String.format(MongoConstants.CAST, String.format(MongoConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName()), MongoConstants.DEFAULT_FLOAT_FORMAT);
                    } else {
                        originField = String.format(MongoConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                    }
                }
    ...
    public String transTreeItem(SQLObj tableObj, DatasetRowPermissionsTreeItem item) {
        ...
        if (field.getDeType() == 1) {
            ...
            if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                whereName = String.format(MongoConstants.CAST, originName, MongoConstants.DEFAULT_FLOAT_FORMAT);
            }
            if (field.getDeExtractType() == 1) {
                whereName = String.format(MongoConstants.UNIX_TIMESTAMP, originName);
            }
            if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                whereName = originName;
            }
    ...
    private SQLObj getYFields(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (StringUtils.equalsIgnoreCase(y.getOriginName(), "*")) {
            ...
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "avg") || StringUtils.containsIgnoreCase(y.getSummary(), "pop")) {
                String cast = String.format(MongoConstants.CAST, originField, y.getDeType() == 2 ? MongoConstants.DEFAULT_INT_FORMAT : MongoConstants.DEFAULT_FLOAT_FORMAT);
                ...
                String cast = String.format(MongoConstants.CAST, originField, y.getDeType() == 2 ? MongoConstants.DEFAULT_INT_FORMAT : MongoConstants.DEFAULT_FLOAT_FORMAT);
                if (StringUtils.equalsIgnoreCase(y.getSummary(), "count_distinct")) {
                    fieldName = String.format(MongoConstants.AGG_FIELD, "COUNT", "DISTINCT " + cast);
                } else {
