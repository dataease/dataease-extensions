package io.dataease.plugins.datasource.maxcompute.query;
...
public class PrestoQueryProvider extends QueryProvider {
    ...
    public String createQueryTableWithPage(String table, List<DatasetTableField> fields, Integer page, Integer pageSize, Integer realSize, boolean isGroup, Datasource ds, List<ChartFieldCustomFilterDTO> fieldCustomFilter) {
        return createQuerySQL(table, fields, isGroup, ds, fieldCustomFilter) + " LIMIT " + realSize;
    }
        ...
        if (CollectionUtils.isNotEmpty(yAxis)) {
            for (int i = 0; i < yAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(y.getExtField()) && y.getExtField() == 2) {
                    ...
                    originField = String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                } else {
                    originField = String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                }
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    if (x.getDeType() == 2 || x.getDeType() == 3) {
                        originField = String.format(PrestoConstants.CAST, String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName()), PrestoConstants.DEFAULT_FLOAT_FORMAT);
                    } else {
                        originField = String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                    }
                }
    ...
    public String convertTableToSql(String tableName, Datasource ds) {
        String schema = new Gson().fromJson(ds.getConfiguration(), JdbcConfiguration.class).getSchema();
        schema = String.format(PrestoConstants.KEYWORD_TABLE, schema);
        return createSQLPreview("SELECT * FROM " + schema + "." + String.format(PrestoConstants.KEYWORD_TABLE, tableName), null);
    }
    ...
    public String transCustomFilterList(SQLObj tableObj, List<ChartFieldCustomFilterDTO> requestList) {
        ...
        for (ChartFieldCustomFilterDTO request : requestList) {
            ...
            if (ObjectUtils.isNotEmpty(field.getExtField()) && field.getExtField() == 2) {
                ...
                originName = String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
            } else {
                originName = String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
            }
            if (field.getDeType() == 1) {
                ...
                if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                    ...
                    whereName = String.format(PrestoConstants.FROM_UNIXTIME, cast);
                }
                ...
                if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                    whereName = String.format(PrestoConstants.CAST, originName, PrestoConstants.DEFAULT_FLOAT_FORMAT);
                }
                if (field.getDeExtractType() == 1) {
    ...
    private SQLObj getXFields(ChartViewFieldDTO x, String originField, String fieldAlias) {
        ...
        if (x.getDeExtractType() == DeTypeConstants.DE_TIME) {
            if (x.getDeType() == 2 || x.getDeType() == 3) {
                ...
                fieldName = String.format(PrestoConstants.FORMAT_DATETIME, originField, format);
            } else {
                fieldName = originField;
            }
        } else {
    ...
    private SQLObj getYFields(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (StringUtils.equalsIgnoreCase(y.getOriginName(), "*")) {
            fieldName = PrestoConstants.AGG_COUNT;
        } else if (SQLConstants.DIMENSION_TYPE.contains(y.getDeType())) {
            fieldName = String.format(PrestoConstants.AGG_FIELD, y.getSummary(), originField);
        } else {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "avg") || StringUtils.containsIgnoreCase(y.getSummary(), "pop")) {
                String cast = String.format(PrestoConstants.CAST, originField, y.getDeType() == DeTypeConstants.DE_INT ? PrestoConstants.DEFAULT_INT_FORMAT : PrestoConstants.DEFAULT_FLOAT_FORMAT);
                String agg = String.format(PrestoConstants.AGG_FIELD, y.getSummary(), cast);
                fieldName = String.format(PrestoConstants.CAST, agg, PrestoConstants.DEFAULT_FLOAT_FORMAT);
            } else {
                String cast = String.format(PrestoConstants.CAST, originField, y.getDeType() == DeTypeConstants.DE_INT ? PrestoConstants.DEFAULT_INT_FORMAT : PrestoConstants.DEFAULT_FLOAT_FORMAT);
                fieldName = String.format(PrestoConstants.AGG_FIELD, y.getSummary(), cast);
            }
        }
        return SQLObj.builder()
                .fieldName(fieldName)
        ...
        if (CollectionUtils.isNotEmpty(y.getFilter()) && y.getFilter().size() > 0) {
            y.getFilter().forEach(f -> {
                ...
                if (StringUtils.equalsIgnoreCase(f.getTerm(), "null")) {
                    ...
                    whereValue = String.format(PrestoConstants.WHERE_VALUE_VALUE, f.getValue());
                }
                list.add(SQLObj.builder()
    ...
    private String calcFieldRegex(String originField, SQLObj tableObj) {
        ...
        for (DatasetTableField ele : calcFields) {
            originField = originField.replaceAll("\\[" + ele.getId() + "]",
                    String.format(PrestoConstants.KEYWORD_FIX, tableObj.getTableAlias(), ele.getOriginName()));
        }
        return originField;
    }
    ...
    public void setSchema(SQLObj tableObj, Datasource ds) {
        if (ds != null && !tableObj.getTableName().startsWith("(") && !tableObj.getTableName().endsWith(")")) {
            PrestoConfig prestoConfig = json.fromJson(ds.getConfiguration(), PrestoConfig.class);
            String schema = prestoConfig.getSchema();
